...
public class Loja {
	...
	public static void main(String[] args) {
		...
		String[] produtos = { "Ovo", "Carne", "Açucar", "Azeite", "Mamão", "Banana", "Limão", "Batata", "Goiaba",
				"Peixe" };
		...
		do {
			...
			if (codigo <= 10) {
				...
				if (estoques[codigo - 1] >= quantidade) {
					...
					System.out
							.printf(quantidade + " un. de " + produtos[codigo - 1] + " foi adicionado ao carrinho!\n");
		...
		for (int i = 0; i < 10; i++) {
			...
			if (carrinho[i][0] > 0) {
				...
				System.out.printf("  %d \t %s\t\t %d\t\t   %.2f\t\t%.2f\n", codigos[i], produtos[i], carrinho[i][1],
						precos[i], precoUnitarioTotal);
